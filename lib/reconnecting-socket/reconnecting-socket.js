/*
  reconnect-socket implements the client-side of the relay protocol.
  It is interface through which all components in chezbob communicate.

  It supports messaging between clients as well as a request/response system
  built on top of that messaging.
*/

// This module is used both from node and from the browser.
// To ensure it works in both cases we need to do a little set up

// Node doesn't have the same CustomEvent type the browser has, so let's make it
let CustomEvent =
...
// Node needs to import its websocket implementation
let WebSocket = globalThis.WebSocket || (await import("ws")).WebSocket;

// Node needs to import its webcrypto implementation
let crypto = globalThis.crypto || (await import("crypto")).webcrypto;
// END isomorphic nonsense
...
export class ReconnectingSocket extends EventTarget {
  ...
  static async connect(server, role) {
    if (role === undefined) {
      debug(() => console.log("No role specified, generating one: ", role = uuid()))
    }
...
export function debug(code) {
  if (globalThis.process && process.env?.DEBUG) {
    code();
  }
}
